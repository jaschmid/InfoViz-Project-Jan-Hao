...
{
    public class SampleGavComponent : GavComponent
    {

        private List<SampleLayer> _sampleLayers;

        public IDataCubeProvider Input
        {
            get { return _sampleLayers[0].Input; }
            set { foreach (SampleLayer layer in _sampleLayers) layer.Input = value; }
        }

        public ColorMap ColorMap
        {
            get { return _sampleLayers[0].ColorMap; }
            set { foreach (SampleLayer layer in _sampleLayers) layer.ColorMap = value; }
        }


        public SampleGavComponent()
            : base()
        {
            _sampleLayers = new List<SampleLayer>();
            ...
            _sampleLayers.Add(glyphLayer);

            TextLayer textLayer = new TextLayer();
            textLayer.Control.SetOuterMargins(10, GavControl.DistanceType.Absolute);
            _sampleLayers.Add(textLayer);

            foreach (SampleLayer layer in _sampleLayers)
            {
                AddLayer(layer);
            }
        }
    ...
    public class SampleLayer : ComponentLayer
    {
    ...
    /// </summary>
    public class GlyphLayer : SampleLayer
    {
    ...
    /// <summary>
    /// Layer that draws one line per item in the dataset. Position in X is spread according to columns and position in Y is based on value.
    /// </summary>
    public class LineLayer : SampleLayer
    {
        // DírectX structure that handles line drawing
        private Line _d3dLine;
        // List containing line vector arrays
        private List<Vector2[]> _lineVectors;

        public LineLayer()
        {
            _lineVectors = new List<Vector2[]>();
        }

        protected void Initialize(Device device)
        {
            _d3dLine = new Line(device);
            _inited = true;
        }
        ...
        protected override void Render(Device device)
        {
            ...
            for (int i = 0; i < _lineVectors.Count; i++)
            {
                // Draws the apropriate line in the color specified by the colormap
                _d3dLine.Draw(_lineVectors[i], ColorMap.GetColor(i).ToArgb());
            }
        }
        ...
        private void CreateLines()
        {
            ...
            {
                _lineVectors.Add(new Vector2[data.GetLength(0)]);
                for (int x = 0; x < data.GetLength(0); x++)
                {
                    _lineVectors[y][x] = new Vector2(columnSpacing * x, Control.AbsoluteSize.Height - (data[x, y, 0] - columnMinList[x]) / (columnMaxList[x] - columnMinList[x]) * Control.AbsoluteSize.Height);
                }
            }
        }
    }
    ...
    /// </summary>
    public class TextLayer : SampleLayer
    {
        ...
        protected void Initialize(Device device)
        {
            ...
            _textMesh = Mesh.TextFromFont(device, _font, "This text is draw using a mesh", 0, 0.5f);
    ...
    /// </summary>
    public class InteractionLayer : SampleLayer
    {
